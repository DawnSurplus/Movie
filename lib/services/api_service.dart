...
class APIService {
  ...
  static const String TMDB_READ_ACCESS_TOKEN = "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI2MGJhNTViOTM0NGY1YmIyYThhOTY2YTU3YzIwMGI1YiIsIm5iZiI6MTczMzY3Njc0NS40MDcsInN1YiI6IjY3NTVjZWM5NmYzN2Y2ZTg5YWRjNWUzNyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.2_8Mnk-0ohnchne58joxjHQt5SrBZPNj6OGhNvecSzE";

  static const String TMDB_BASE_URL = "https://api.themoviedb.org/3/movie";
  static const String TMDB_QUERY  = "language=ko-KO";
  ...
  static Future<List<ResultModel>> getPopularMovie() async {
    final url = Uri.parse("$TMDB_BASE_URL/popular?$TMDB_QUERY");
  ...
  static Future<List<ResultModel>> getNowPlayingMovie() async {
    final url = Uri.parse("$TMDB_BASE_URL/now_playing?$TMDB_QUERY");
    final response = await http.get(url, headers: TMDB_HEADER);
  ...
  static Future<List<ResultModel>> getComingSoonMovie() async {
    final url = Uri.parse("$TMDB_BASE_URL/upcoming?$TMDB_QUERY");
    final response = await http.get(url, headers: TMDB_HEADER);
